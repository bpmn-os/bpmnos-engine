#ifndef BPMNOS_Execution_GreedyCandidateDispatcher_H
#define BPMNOS_Execution_GreedyCandidateDispatcher_H
 ...
 * feasible one — the front of the set, unless that front is infeasible. A future RolloutDispatcher will
 * compose the same sources with a different policy.
 *
 * (Temporary name; renamed to GreedyDispatcher once the old dispatchers are removed.)
 */
template <typename Source>
class GreedyCandidateDispatcher : public EventDispatcher {
public:
  template <typename... Args>
  GreedyCandidateDispatcher(Args&&... args) : source(std::forward<Args>(args)...) {}
...
#endif // BPMNOS_Execution_GreedyCandidateDispatcher_H
