...
class StateMachine {
public:
  StateMachine(const SystemState* systemState, const BPMN::Process* process, const Values& status);
  StateMachine(const SystemState* systemState, const BPMN::Scope* scope, const Values& status, Token* parentToken);
