...
//  std::vector< Token* > tokensAtActiveResources; ///< Container holding indices of tokens at busy resources
  ...
  std::unordered_map< Token*, std::vector<Token*> > tokensAtActivityInstance; ///< Map holding all tokens representing an instance of a multi-instance (or loop) activity for each token waiting at the multi-instance (or loop) activity.
