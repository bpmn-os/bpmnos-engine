...
//std::cerr << "shutdown: " << scope->id << std::endl;
  ...
  if ( auto eventSubProcess = scope->represents<BPMN::EventSubProcess>() ) {
    ...
    const_cast<SystemState*>(systemState)->objective += scope->extensionElements->as<BPMNOS::Model::ExtensionElements>()->getContributionToObjective(mergedStatus,data,parentToken->globals);
